...
The site is developed using a reactive framework thereby allowing a dynamic interaction with any of the input variables.

If you have any questions, or would like to provide feedback please:

- [email me](mailto:serverid@yahoo.com")
...
renderPlotly({
  ...
  subplot(boxplotG, histogramG, nrows = 2, shareX = TRUE) %>%
    layout(xaxis = list(
           tickformat = "%"),
      title = "ROI with Outliers", bargap = 0.1, showlegend = FALSE)
})
```
...
renderPlotly({
  ...
  subplot(boxplotD, histogramD, nrows = 2, shareX = TRUE) %>%
    layout(showlegend = FALSE) %>%
    layout(title = "ROI with Outliers", bargap = 0.1)
})
```
...
```{r 2.5.2.1-box-tradeDays, eval=trend}
xhp <- list(title = d$catName, titlefont = f)
yhp <- list(title = "Counts", titlefont = f)
ybp <- list(title = d$catName, titlefont = f)
