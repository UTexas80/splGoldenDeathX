...
chart.Boxplot(data.table(returnsByCategory) %>% select(starts_with("D")))
chart.Boxplot(data.table(returnsByCategory) %>% select(starts_with("Golden")))
...
renderPlotly({
  histogramD <- plot_ly(d, x = d$tradeDays, type = "histogram", marker = list(color = ~count)) %>%
    layout(xaxis = xhp, yaxis = yhp)
  
  boxplotD <- plot_ly(d,x = d$tradeDays, type = "box", color = "black", boxpoints = "all", jitter = 0.3, marker = list(
                    color = "black",
                    outliercolor = "black",
                    line = list(
                      outliercolor = "black",
...
renderPlotly({
  ...
  boxplotG <- plot_ly(g, x = g$tradeDays, type = "box", boxpoints = "all", jitter = 0.3) %>%
    layout(yaxis = ybpG)  
 subplot(boxplotG,histogramG,nrows = 2, shareX = TRUE) %>%  layout(showlegend = FALSE)  %>%
