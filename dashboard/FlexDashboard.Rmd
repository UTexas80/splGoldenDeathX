...
emaPct                    <- data.table(readRDS(here::here("dashboard/rds", "emaPct.rds")))
smaPct                    <- data.table(readRDS(here::here("dashboard/rds", "smaPct.rds")))  
...
DT::renderDataTable({
    ...
    trendTable <- trendTable[catName == "GoldenX" & 
                        (startDate >= input$dateRange2.2[1] & 
                         endDate <= input$dateRange2.2[2])][,-9]
...
```{r 2.2.2.2-viz-goldenx-posn-chartstats}
    ...
    renderPlot({
      ind <- tolower(unique(trend[,1])[(as.numeric(input$radio2.2) - 2),]) 
      chart.Posn(paste0("gX", ind), 
       Symbol = "SPL.AX",  
...
```
### Trade Stats
```{r 2.2.2.3-table-goldenX-tradestats}
# goldenX_EMA_tradestats <- blotter::tradeStats(
#   "goldenX_EMA_portfolio", 
...
```{r 2.3.1.2-deathx-vbox-returns-annualy}
flexdashboard::renderValueBox({
  flexdashboard::valueBox(
    paste(
      as.character(
...
DT::renderDataTable({
    ...
    trendTable <- trendTable[catName == "DeathX" & 
                        (startDate >= input$dateRange2.3[1] & 
                         endDate <= input$dateRange2.3[2])][,-9]
    DT::datatable(trendTable,
    rownames = FALSE,
    extensions = c("Scroller"),
    options = list(
...
```{r 2.3.2.2-viz-deathX-posn-chartstats}
    ...
    renderPlot({
      ind <- tolower(unique(trend[,1])[(as.numeric(input$radio2.3) - 0),]) 
      chart.Posn(paste0("dX", ind), 
       Symbol = "SPL.AX",  
...
```{r 2.4.2.1-viz-nox-posn-chartstats}
    ...
    renderPlot({
      ind <- tolower(unique(trend[,1])[(as.numeric(input$radio2.4) - 4),]) 
      chart.Posn(paste0("nX", ind), 
       Symbol = "SPL.AX",  
...
DT::renderDataTable({
    ...
    trendTable <- trendTable[catName == "NoX" & 
                        (startDate >= input$dateRange2.4[1] & 
                         endDate <= input$dateRange2.4[2])][,-9]
    DT::datatable(trendTable,
    rownames = FALSE,
    extensions = c("Scroller"),
    options = list(
...
```{r 2.4.3.2-viz-noX-trade-days, eval=trend}
xhp <- list(title = n$catName, titlefont = F)
yhp <- list(title = "Counts", titlefont = F)
ybp <- list(title = n$catName, titlefont = F)
