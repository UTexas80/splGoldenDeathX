...
```{r processing}
trendname    <- str_sub(tail(trend[order(endDate)], 1)[, 1], end = -4)
...
```{r 2.1.0.1-vbox-trend, eval=trend}
...
```{r 2.1.0.2-vbox-returns-roi}
trendROI <- percent(tail(trend[order(endDate)], 1)[, 5][[1]],
...
```{r 2.1.0.3-vbox-trade-days, eval=trend}
flexdashboard::renderValueBox({
  flexdashboard::valueBox(
    paste("Trade Days:", " ", format(tail(trend[order(endDate)], 1)[, 4], big.mark = ",")),
    color = # 0000ff
    , icon = "fa-calendar"
  )
})
