...
This site is developed using a reactive framework thereby allowing a dynamic interaction with any of the input variables. Therefore, whenever the date is updated the corrresponding entries will be dynamically updated as well. As a sidenote, to formulate the Crossing trading systems, the data is based upon 20, 50, 100 and 200 day moving averages. Hence any dates selected with a start date greater than 200 days prior to today will currently generate an error.
...
price <- reactive({
  tidyquant::tq_get("SPL.AX", from = format(input$dateRange[1]), to = format(input$dateRange[2])) %>%
    mutate(
      change = close - open,
...
```{r 2.1.1.b0-viz-box-returns, eval=trendReturns}
dRetEMA <- trendReturnsEMAlong[trend %like% "^D"][order(-returns)]
gRetEMA <- trendReturnsEMAlong[trend %like% "^G"][order(-returns)]
nRetEMA <- trendReturnsEMAlong[trend %like% "^n"][order(-returns)]


dRetEMA[,`:=`(
  key = substr(trend, 1, 1),
  N = .N,
  Mean = mean(returns),
  SDev = sd(returns),
  ...
  Min = min(returns),
  Max = max(returns),
  AboveAvg = ifelse(returns - mean(returns) > 0, TRUE, FALSE))]
gRetEMA[,`:=`(
  key = substr(trend, 1, 1),
  N = .N,
  Mean = mean(returns),
...
retEMA <- rbind(dRetEMA,gRetEMA, nRetEMA)
renderPlotly({
  ggplotly( 
     ggplot(dRetEMA, aes(returns, trend, color = AboveAvg))
    + scale_x_continuous(labels = percent)
    + geom_segment(aes(x = Mean, y = trend, xend = returns, yend = trend), color = "grey50")
    + geom_point())  %>%
  subplot(ggplot(gRetEMA, aes(returns, trend, color = AboveAvg))
    + scale_x_continuous(labels = percent)
    + geom_segment(aes(x = Mean, y = trend, xend = returns, yend = trend), color = "grey50")
    + geom_point()
    )   %>%
  subplot(ggplot(nRetEMA, aes(returns, trend, color = AboveAvg))
    + scale_x_continuous(labels = percent)
    + geom_segment(aes(x = Mean, y = trend, xend = returns, yend = trend), color = "grey50")
    + geom_point()
...
```{r 2.1.1.b2-viz-box-returns, eval=trendReturns}
dRetEMA <- trendReturnsEMAlong[trend %like% "^D"][order(-returns)]
gRetEMA <- trendReturnsEMAlong[trend %like% "^G"][order(-returns)]
nRetEMA <- trendReturnsEMAlong[trend %like% "^n"][order(-returns)]
...
renderPlotly({
  ggplotly( 
     ggplot(retEMA, aes(returns, trend, color = AboveAvg))
    + scale_x_continuous(labels = percent)
    + geom_segment(aes(x = Mean, y = trend, xend = returns, yend = trend), color = "grey50")
    + geom_point()
...
Row {data-height=33%}
-------------------------------------
### Trend Return Distribution
```{r  2.1.2.1-viz-vboxplot, eval=trend}
renderPlotly({
  ggplotly(
    ggplot(trend, aes(x = paste(trend$catName, "-", trend$indicator), y = return))
...
renderPlotly({
  ...
  boxplotG <- plot_ly(g, x = g$return, type = "box", boxpoints = "all", jitter = 0.3, marker = list(color = "teal", outliercolor = "blue")) %>%
    layout(yaxis = ybpG)
    ...
    paste(
      as.character(
        percent(
          Return.annualized(
            na.omit(
              trendReturnsDaily[,as.numeric(input$radio2.3)])[paste(format(input$dateRange2.3[1]),
    ...
    paste(
      as.character(
        percent(
          as.numeric(
            Return.cumulative(
              na.omit(trendReturnsDaily[,as.numeric(input$radio2.3)])[paste(format(input$dateRange2.3[1]),
                format(input$dateRange2.3[2]),sep = "::")]
            )
          )
        )
