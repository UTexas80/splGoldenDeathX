...
# catName        <-unique(substr(aa$catName,1,nchar(aa$catName)-3))
trendname      <-str_sub(tail(aa[order(endDate)],1)[,1],end=-4)
...
renderHighchart({
    highchart() %>% 
      hc_chart(type = "column") %>% 
        hc_add_series_labels_values(labels = trendStats$catName,
                                    values = trendStats$N) %>%
        hc_add_theme(hc_theme_darkunica())
})
```
