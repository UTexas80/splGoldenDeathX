...
flexdashboard::renderValueBox({
  flexdashboard::valueBox(
      paste("Calendar Days:", " ", format(tail(trend[order(endDate)],1)[,4], big.mark = ","))
      ,color = #0000ff
      ,icon = "fa-calendar"
  )
})
...
### Trend boxplot
```{r  2.1.2.1-viz-boxplot, eval=trend}
renderPlotly({
    ggplotly(
      ggplot(aa, aes(x = substr(aa$catName,1,nchar(aa$catName)-3), y = return))
