...
radioButtons("radioMA", "Moving Avg:",
               c("Exponential" = "EMA",
                 "Simple" = "SMA")
             )
```
...
flexdashboard::renderValueBox({
  flexdashboard::valueBox(
    paste(as.character(percent(as.numeric(Return.annualized(trendReturnsDaily)[,3]))), "", " Annual Return"),
    color = ifelse(as.numeric(Return.annualized(trendReturnsDaily)[,3]) > 0, "teal", "red"),    
    icon = "ion-cash"
  )
})
```
...
flexdashboard::renderValueBox({
  flexdashboard::valueBox(
    paste(as.character(percent(as.numeric(Return.cumulative(trendReturnsDaily)[,3]))), "", " Total Return"),
    color = ifelse(as.numeric(Return.cumulative(trendReturnsDaily)[,3]) > 0, "teal", "red"),    
    icon = "ion-cash"
  )
})
```
...
flexdashboard::renderValueBox({
  tradeDays <- format(trend[catName == "GoldenX", sum(tradeDays), by = catName][,2], big.mark = ",")
  flexdashboard::valueBox(
...
renderTable(
Return.cumulative(na.omit(trendClose[,c(4)]))[paste(format(input$dateRange2.2[1]),format(input$dateRange2.2[2]),sep = "::")]
)
```
