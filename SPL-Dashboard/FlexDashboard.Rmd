...
SPL.FI   <- FinancialInstrument::getInstrument("SPL.AX")
SPL.FI   <- get("SPL.AX", envir  = FinancialInstrument:::.instrument)
# ------------------------------------------------------------------------------
```
...
renderPlot({
  x <- tk_xts(price())
  chartSeries(x, theme = 'black',subset = '2014::2020', TA = "addBBands();addVo();addCMF();addZLEMA()")   #Year 2014 To Present
})
...
```{r 1.2.1.8-viz-forecast-ets, eval=price}
renderPlot({
  etsPrice <- tk_xts(price())
  plot(forecast(etsPrice[,4]))
})
