...
flexdashboard::renderValueBox({
  xts_price <- tk_xts(price())
  annReturn <- percent((as.numeric(last(xts_price[, 4])) / as.numeric(first(xts_price[, 4])))^((1 / nyears(xts_price))) - 1, accuracy = 0.1)
...
flexdashboard::renderValueBox({
  xts_price <- tk_xts(price())
  cumReturn <- percent((as.numeric(last(xts_price[, 4])) - as.numeric(first(xts_price[, 4]))) / (as.numeric(first(xts_price[, 4]))), accuracy = 0.1)
...
renderPlot({
  ...
  x <- tk_xts(price())
  y <- max(SPL.AX[,6], na.rm = TRUE)
  
  # Ensure breaks span the range of x
  max_val <- max(x[,6], na.rm = TRUE)
  upper_limit <- max(y, max_val)
  ...
  hist(x[,6],
    ...
    freq = TRUE,
    border = "white",
    xlim = c(0.0, upper_limit),
    breaks = seq(0, upper_limit + 0.5, by = 0.10)
  )
})
```
...
trendROIema <- percent(tail(trend[order(endDate)][indicator == 'EMA'],1)[,8][[1]], accuracy = 0.1)
...
trendROI <- percent(tail(trend[order(endDate)][indicator == 'SMA'],1)[,8][[1]], accuracy = 0.1)
...
m <- setDT(melt(trendReturnsAnnualized[1,], id.vars = c("rn"))[,-1])[-7,]
names(m) <- c("trend", "return")
m <- m[, `:=`(trend=str_sub(trend, end = -4), indicator= str_sub(trend, -3, -1))]
...
cumReturn <- percent(as.numeric(Return.cumulative(trend[,9])), accuracy = 0.1)
flexdashboard::renderValueBox({
  flexdashboard::valueBox(
    paste(
      as.character(
