...
# ------------------------------------------------------------------------------
dXema                     <- blotter::put.portfolio("dXema", gXema, envir = .GlobalEnv)
dXsma                     <- blotter::put.portfolio("dXsma", gXema, envir = .blotter)
# ------------------------------------------------------------------------------
...
gXsma                     <- blotter::put.portfolio("gXsma", gXema, envir = .blotter)
# ------------------------------------------------------------------------------
nXema                     <- blotter::put.portfolio("nXema", gXema, envir = .blotter)
nXsma                     <- blotter::put.portfolio("nXsma", gXema, envir = .blotter)
# ------------------------------------------------------------------------------
```
...
# ------------------------------------------------------------------------------
# curr   <- get("USD", envir=FinancialInstrument:::.instrument)
...
m<- FinancialInstrument::loadInstruments(here::here("m.RData"))
# ------------------------------------------------------------------------------
...
```{R blotterFunctions, include=FALSE, warning=FALSE, cache=FALSE}
# log chart in blotter https://tinyurl.com/yaqbe2yw

log.chart.Posn <- function(Portfolio, Symbol, Dates = NULL, env = .blotter) {
  pname<-Portfolio
  # Portfolio<-getPortfolio(pname)
  #Portfolio<-getPortfolio(p1)
  # Portfolio                         <- na.omit(readRDS(here::here("dXsma_port_rds.rds")))
  Portfolio                         <- gXema
#  x <- get(Symbol, env)
  x <- tk_xts(price())
  Prices <- log(x)
  chart_Series(na.omit(Prices))
...
```{r 1.1.2.2-table-portfolio}

pname <- "dXsma"
dXsma_port_rds                    <- readRDS(here::here("dXsma_port_rds.rds"))
dxsma_summary                     <- dXsma_port_rds$summary

DT::renderDataTable({
  DT::datatable(dxsma_summary,
    rownames = FALSE,
    extensions = c("Scroller"),
    filter = 'top',
    options = list(
...
```{r 2.2.2.0-viz-goldenx-posn-chartstats, eval=gXema}
...
#    load.strategy("gXema")
#    quantstrat::get.strategy(paste0("gX", ind), envir = .strategy)
#    FinancialInstrument::stock("SPL.AX", "USD")
#    FinancialInstrument::getInstrument("SPL.AX")
#    get("AUD", envir=FinancialInstrument:::.instrument)
#    get("SPL.AX", envir=FinancialInstrument:::.instrument)
...
### Price & Transactions Chart - tk_xts
```{r 2.2.2.1-viz-goldenx-posn-chartstats, eval=gXema}
...
#    get("AUD", envir=FinancialInstrument:::.instrument)
#    get("SPL.AX", envir=FinancialInstrument:::.instrument)
    na.omit(getSymbols("SPL.AX", src='FI', dir=".", split_method='common', env=.instrument))
    
    SPL.AX <- tk_xts(price())[,1:7]
    blotter::chart.Posn(paste0("portfolio.", "gX", ind),
      Symbol = "SPL.AX",
      Dates  = paste(format(input$dateRange2.2[1]), format(input$dateRange2.2[2]),sep = "::"),
      env    = .instrument,
      TA     = "add_SMA(n = 20, col = 2); add_SMA(n = 50, col = 4)")
...
### SPL Table
```{r 2.2.2.1-viz-table-spl-ax}

DT::renderDataTable({
  spl <- data.table(getSymbols("SPL.AX", src='FI', dir=".", split_method='common', env=.instrument), keep.rownames = TRUE)
  xts_price <- data.table(tk_xts(price()), keep.rownames = TRUE)[,1:7]
  DT::datatable(xts_price,
    rownames = FALSE,
    extensions = c("Scroller"),
    options = list(
      pageLength = 10,
...
### Price & Transactions Chart Part II
```{r 2.2.2.4-viz-goldenx-posn-chartstats}
  
  renderPlot({
    ind <- tolower(data.table::setorder(unique(trend[,1]))[(as.numeric(input$radio2.2) - 2),])
    log.chart.Posn(Portfolio = paste0("gX", ind), 
                   Sym = "SPL.AX", 
                   Dates  = paste(format(input$dateRange2.2[1]), format(input$dateRange2.2[2]),sep = "::"),
                   env = .GlobalEnv)
#      add_MACD()
...
### Blotter
```{r 2.2.2.5-table-blotter}
  
DT::renderDataTable({
  fi <- data.table(ls(envir = .blotter))
  DT::datatable(fi,
    rownames = FALSE,
    extensions = c("Scroller"),
    options = list(
...
```{r 2.2.2.6-table-financial-instruments}
...
### Instruments
```{r 2.2.2.7-table-financial-instruments}
  
DT::renderDataTable({
  i <- as.list(FinancialInstrument:::.instrument, all.names=TRUE)
  DT::datatable(matrix(unlist(i)),
    rownames = FALSE,
    extensions = c("Scroller"),
    options = list(
      pageLength = 10,
...
### Symbols
```{r 2.2.2.8-table-portfolio-symbols}
  
DT::renderDataTable({
  ind <- data.table::setorder(unique(trend[,1]))[(as.numeric(input$radio2.2) - 2),]
  p  <- blotter::getPortfolio(paste0("gXema"), envir = .blotter)
  get("AUD", envir=FinancialInstrument:::.instrument)
  get("SPL.AX", envir=FinancialInstrument:::.instrument)
  s <- data.table(ls(p$symbols$SPL.AX$txn))
  DT::datatable(s,
    rownames = FALSE,
    extensions = c("Scroller"),
    options = list(
...
### Strategy
```{r 2.2.2.9-table-portfolio-strategy}
  
DT::renderDataTable({
  load.strategy("gXema")
  strat <- data.table(ls(envir = .strategy))
  DT::datatable(strat,
    rownames = FALSE,
    extensions = c("Scroller"),
    options = list(
