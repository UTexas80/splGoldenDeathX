...
add.signal(strategy.st,
    ...
    arguments               = list
        (columns            = c("SMA.020","SMA.050","SMA.100", "SMA.200"),
        formula             = str_replace_all(dXsma_close, "&", "|"),
         label              = "trigger",
         cross              = TRUE),
...
dXsma_trend <- na.omit(dXsma_trend[SPL, nomatch = 0][, -c(26,28:32)])
setkey(dXsma_trend, "End")
dXsma_trend <- na.omit(dXsma_trend[SPL, nomatch = 0][, -c(27,29:33)])
################################################################################
# 11.0	# Performance and Risk Metrics 
################################################################################
dXsma_rets           <- PortfReturns(Account =  account.st)
rownames(dXsma_rets) <- NULL
names(dXsma_rets)[1] <- strategy.st
# ------------------------------------------------------------------------------
